  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      if (user) {
        ...
        if (wallsData) {
          const actualTotal = wallsData.reduce(
            (acc, wall) => acc + (wall.messages?.length || 0),
            0,
          );
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="flex justify-between items-center mb-10">
          ...
          <motion.button
            ...
            className="p-3 text-gray-400 hover:text-rose-500 transition-colors glass-card rounded-2xl"
          >
            <LogOut size={20} />
          </motion.button>
        </header>
          ...
          >
            <div>
              ...
              <h2 className="text-3xl font-black text-gray-900 leading-tight tracking-tighter">
                Your Digital Celebration Hub
              </h2>
              <p className="text-gray-500 font-medium mt-2 max-w-md">
                Create a wall for any occasion—Valentines, Birthdays,
                Graduations, and more.
              </p>
            </div>
            <Link
              href="/create"
              className="mt-8 px-8 py-4 bg-gray-900 text-white rounded-2xl font-bold inline-flex items-center gap-2 shadow-xl hover:bg-black transition-all"
            >
            ...
            {walls.map((wall, index) => (
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${wall.type === "valentine" ? "bg-rose-50 text-rose-500" : "bg-blue-50 text-blue-500"}`}
                  >
                    {wall.type === "valentine" ? (
                  ...
                  <div className="flex gap-1">
                    <motion.button
                      whileTap={{ scale: 0.9 }}
                      onClick={() => handleShare(wall.slug, wall.type)}
                      className="p-2.5 hover:bg-white/50 rounded-xl text-gray-400"
                    >
                      <Share2 size={18} />
                    </motion.button>
                    <motion.button
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-1 leading-tight tracking-tighter">
                  {wall.name}
                </h3>

                <div
                  className={`rounded-full p-2 mb-6 border ${wall.type === "valentine" ? "bg-rose-50/50 border-rose-100" : "bg-blue-50/50 border-blue-100"}`}
                >
                  <p
                    className={`text-[10px] font-black uppercase text-center ${wall.type === "valentine" ? "text-rose-400" : "text-blue-400"}`}
                  >
                    {wall.messages.length > 0
                      ? `${wall.messages.length} New Messages`
                ...
                <div className="mt-auto pt-6 border-t border-white/50 flex justify-between items-center">
                  <Link
                    href={`/wall/${wall.slug}`}
                    className="flex items-center gap-2 text-sm font-bold text-gray-900 hover:text-rose-500 group/link"
                  >
                  ...
                  <motion.button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-gray-900 text-white rounded-xl text-xs font-bold"
                  >
                    {copiedId === wall.id ? (
                      <Check size={14} />
                    ) : (
