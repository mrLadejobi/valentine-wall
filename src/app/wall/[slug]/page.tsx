...
export default function WallPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleSocialInvite = async () => {
    const text = wall?.type === 'valentine' ? "Secret valentines! Locked till Feb 14, 2026! 🤫💌" : "Digital yearbook notes! Locked until graduation! 🎓✨";
    if (navigator.share) {
      ...
      <main className="h-screen flex flex-col items-center justify-center p-8 text-center relative">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="z-10 glass-card p-12 rounded-[48px] max-w-xs w-full border-white shadow-2xl">
          ...
          <motion.button 
            ...
            className={`w-full py-5 rounded-full text-white font-black text-xl shadow-2xl ${currentTheme.accent}`}
          >
            Enter & Play Vibe
          </motion.button>
        </motion.div>
      ...
      <div className="max-w-4xl mx-auto min-h-screen flex flex-col relative">
          ...
          {isUnlocked && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className={`w-full py-4 text-center font-black text-white relative overflow-hidden shadow-inner ${wall.type === 'valentine' ? 'bg-rose-500' : wall.type === 'graduation' ? 'bg-amber-500' : 'bg-blue-500'}`}>
              <motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="relative z-10 flex items-center justify-center gap-3 text-lg">
                <Sparkles size={18} /> {wall.type === 'valentine' ? "Happy Valentine's Day! ❤️" : `Happy Graduation Day, ${wall.name}! 🎓`} <Sparkles size={18} />
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {view === 'wall' ? (
          <>
              ...
              <div className="glass-card rounded-[40px] p-8 flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl">
                <div className="text-center md:text-left flex-1">
                  ...
                  <div className="mt-4 flex flex-wrap justify-center md:justify-start gap-3">
                    ...
                    <div className="bg-white/40 backdrop-blur-md rounded-2xl px-4 py-2 border border-white/50 text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                       <Mail size={12} /> {messages.length} Letters
                    </div>
                  </div>
                </div>
                
                <div className="flex gap-2">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} onClick={handleCopyLink} className="p-4 glass-card rounded-3xl text-rose-500 shadow-lg">{copied ? <Check size={24} className="text-green-500" /> : <Copy size={24} />}</motion.button>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} onClick={handleSocialInvite} className="p-4 glass-card rounded-3xl text-rose-500 shadow-lg"><Share2 size={24} /></motion.button>
                  {isOwner && wall.type === 'graduation' && (
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} onClick={handleDownloadQR} className="p-4 glass-card rounded-3xl text-amber-600 shadow-lg" title="Get Cap QR Code"><QrCode size={24} /></motion.button>
                  )}
                </div>
              </div>
            </header>

            {/* GRADUATION PROMPT SECTION */}
            <AnimatePresence>
              ...
              <AnimatePresence>
                {messages.map((msg) => (
                  <motion.button
                    ...
                    whileHover={{ scale: 1.1, rotate: 0, zIndex: 50, y: -5 }}
                    key={msg.id}
                    onClick={() => setActiveMessage(msg)}
                    className="aspect-4/5 rounded-4xl glass-card relative flex flex-col items-center justify-center overflow-hidden transition-all duration-500 group shadow-xl"
                  >
                    ...
                    {canOpen ? (
                       <span className="text-5xl drop-shadow-2xl z-10">{STAMPS[msg.stamp as StampType].icon}</span>
                    ) : (
                      <div className="z-10 flex flex-col items-center gap-3">
                        <div className="w-14 h-14 rounded-full bg-white/40 flex items-center justify-center backdrop-blur-md border border-white shadow-sm"><Lock size={24} className="text-gray-600 opacity-60" /></div>
                        <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Sealed</span>
                      </div>
                    )}
            ...
            {messages.length === 0 && (
              <div className="text-center py-20 glass-card rounded-[48px] m-6">
                <Mail className="mx-auto mb-4 text-rose-200" size={60} />
                <p className="font-black text-gray-400 uppercase text-xs tracking-widest leading-loose">Waiting for the first<br/>secret envelope...</p>
              </div>
            )}

            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-30 w-full max-w-xs px-6">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setView('write')} className="w-full py-6 rounded-full shadow-2xl bg-gray-900 text-white font-black text-xl flex items-center justify-center gap-3 border-t border-white/20">
                <Send size={24} /> Write Note
              </motion.button>
            </div>
          </>
        ) : (
...
function WriteView({ wallId, theme, onCancel, onSuccess }: { wallId: string; theme: ThemeConfig; onCancel: () => void; onSuccess: () => void }) {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-100 flex items-center justify-center p-6 bg-white/10 backdrop-blur-3xl">
        <div className="max-w-sm w-full glass-card p-12 rounded-[48px] text-center border-white/50 shadow-2xl">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', damping: 12 }} className="w-20 h-20 bg-green-500 text-white rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-lg rotate-12">
          ...
          <div className="space-y-3">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => router.push('/auth')} className="w-full py-5 bg-gray-900 text-white rounded-2xl font-black text-lg shadow-xl">Create My Own Wall</motion.button>
            <button onClick={onSuccess} className="text-xs font-black text-gray-400 uppercase tracking-widest hover:text-rose-500 transition-colors">Back to Mailbox</button>
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-100 bg-white/20 backdrop-blur-3xl flex flex-col items-center overflow-y-auto pb-20"
    >
      <div className="w-full max-w-3xl px-6 py-10 flex items-center justify-between">
        ...
        <div className="text-center flex flex-col items-center">
           <span className="text-[10px] font-black text-rose-500 uppercase tracking-[0.4em] mb-1">Confidential</span>
           <h3 className="font-black text-2xl tracking-tighter text-gray-900">Write Secret Note</h3>
        </div>
        <div className="w-12" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-3xl px-6 space-y-8">
        <div className="relative group">
          <div className={`w-full glass-card rounded-[48px] p-12 transition-all duration-500 shadow-2xl ${isInappropriate ? 'border-red-500/50 bg-red-50/30' : 'border-white/60 bg-white/40'}`}>
            <textarea
              required
              maxLength={5000}
              placeholder="Start typing your secret message here..."
              className="w-full h-112.5 bg-transparent border-none resize-none focus:ring-0 text-2xl font-serif italic text-gray-800 placeholder:text-gray-300 leading-relaxed"
              value={body}
              onChange={(e) => setBody(e.target.value)}
            />
            <div className="flex justify-between items-center mt-6 pt-6 border-t border-black/5">
              <div className="flex items-center gap-3">
                <div className={`w-2.5 h-2.5 rounded-full ${isInappropriate ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`} />
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{isInappropriate ? 'Safety Warning' : 'Safe to send'}</span>
              </div>
              <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-white/40 px-3 py-1 rounded-full">{body.length.toLocaleString()} / 5,000</div>
            </div>
          </div>
          <AnimatePresence>
            {isInappropriate && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="absolute -top-4 left-1/2 -translate-x-1/2 bg-red-600 text-white px-8 py-2 rounded-full text-xs font-black uppercase tracking-widest shadow-2xl">Be Kind ❤️</motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          <div className="glass-card rounded-[40px] p-8 space-y-6 shadow-xl">
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <input placeholder="From (Optional)" className="w-full pl-12 pr-4 py-5 bg-white/30 border border-white/50 rounded-2xl outline-none focus:bg-white/60 transition-all font-bold text-sm" value={author} onChange={(e)=>setAuthor(e.target.value)} />
              </div>
              <div className="relative">
                <Sparkles className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                <input required placeholder="Guess who? (Hint)" className="w-full pl-12 pr-4 py-5 bg-white/30 border border-white/50 rounded-2xl outline-none focus:bg-white/60 transition-all font-bold text-sm" value={hint} onChange={(e)=>setHint(e.target.value)} />
              </div>
            </div>
          </div>

          <div className="glass-card rounded-[40px] p-8 space-y-6 shadow-xl flex flex-col items-center">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Choose a Seal</label>
            <div className="flex flex-wrap justify-center gap-4">
              {(Object.keys(STAMPS) as StampType[]).map((s) => (
                <button 
                  key={s} type="button" 
                  onClick={() => setStamp(s)} 
                  className={`w-16 h-16 rounded-3xl border-2 transition-all flex items-center justify-center text-3xl ${stamp === s ? 'border-rose-500 bg-white scale-110 shadow-2xl' : 'border-white/50 bg-white/10 opacity-40 hover:opacity-100'}`}
                >
                  {STAMPS[s].icon}
                </button>
              ))}
        ...
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={sending || isInappropriate} className="w-full py-8 rounded-[40px] text-white font-black text-3xl shadow-2xl bg-gray-900 hover:bg-black transition-all flex items-center justify-center gap-4 shadow-gray-400 mb-10">
          {sending ? <Loader2 className="animate-spin" /> : <>Seal & Send <Send size={28} /></>}
        </motion.button>
      </form>
    </motion.div>
  );
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" onClick={onClose}>
      <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-lg bg-white/80 backdrop-blur-2xl rounded-[48px] overflow-hidden shadow-2xl border border-white" onClick={e => e.stopPropagation()}>
        <div className={`h-40 ${theme.bg} flex items-center justify-center relative`}>
          ...
          <button onClick={onClose} className="absolute top-6 right-6 p-3 bg-white/20 rounded-full text-gray-600 hover:bg-white/40 transition-colors"><X size={24}/></button>
        </div>
